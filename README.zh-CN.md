...
`get_kimi_task` 适合立即查询，或保留兼容性的 30 秒以内短等待。正常执行中的任务应使用 `wait_kimi_task`：它会忽略中间阶段更新，只在任务结束或等待超时时返回，默认等待 300 秒。真实 Codex 主机实测 120 秒和 300 秒 MCP 调用都能完整返回，没有在 60 秒被截断。

任务仍在运行时，两个查询工具只返回任务 ID、状态、阶段、更新时间、终态标记和 60 秒的兜底轮询建议。任务进入终态后，才返回完整任务、尝试记录、总结或错误，以及 Git 变更收据。这样能减少重复状态结构带来的额度消耗，同时保留最终验收所需的全部证据。
